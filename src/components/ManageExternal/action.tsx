...
/**
 * Busca uma lista paginada de usuários externos do backend.
 ...
 * @returns {Promise<{data?: any[], totalPages?: number, currentPage?: string, totalExternals?: number, error?: string}>}
 * Uma Promise que resolve para um objeto contendo os dados dos usuários, informações de paginação, ou uma mensagem de erro.
 */
export async function getExternals(formData: FormData): Promise<{
  data?: any[];
  totalPages?: number;
  currentPage?: string;
  ...
  try {
    ...
    // Faz uma requisição GET para o endpoint de usuários externos.
    ...
    console.error('Erro na Server Action ao buscar usuários externos:', errorMessage);
